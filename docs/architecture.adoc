...
│       ├── nginx.conf          # Nginx config (K8s version)
│       ├── prometheus.yml      # Prometheus config (K8s version)
...
[cols="1,1,1,1"]
|===
| Deployment         | Image                                | Port  | Readiness Probe
| frontend           | ghcr.io/.../frontend:latest          | 443   | —
