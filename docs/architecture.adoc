...
graph TD
    ...
    subgraph Docker["Docker Compose — greenops.local"]
        ...
        subgraph Services["Internal Services"]
            Alerts["alerts-service (Alert API)"]
            User["user-service (User API)"]
        end
        ...
        Nginx -->|"/api/alerts"| Alerts
        Nginx -->|"/api/user"| User
        Nginx -->|"/api/prometheus"| Prometheus

        Alerts --- Postgres
        User --- Postgres
        PgExporter --- Postgres

        Prometheus -->|"scrape :3001/metrics"| Alerts
        Prometheus -->|"scrape :3003/metrics"| User
        Prometheus -->|"scrape :8080/metrics"| Keycloak
        Prometheus -->|"scrape :9187/metrics"| PgExporter
        Prometheus -->|"self-scrape"| Prometheus

        Alerts -->|"query API"| Prometheus
    end
....
...
sequenceDiagram
    ...
    Note over Browser,PG: Unauthenticated — SPA loads

    Browser->>Nginx: GET https://greenops.local
    Nginx->>Frontend: Serve SPA
    Frontend-->>Browser: Sign in button (Keycloak)

    Browser->>Nginx: GET https://accounts.greenops.local
    Nginx->>Keycloak: OIDC login

    Note over Browser,PG: Authentication

    Keycloak-->>Browser: Login form
    Browser->>Keycloak: Credentials
    Keycloak->>Browser: JWT (access_token)

    Note over Browser,PG: Authenticated API call

    Browser->>Nginx: API + Authorization: Bearer &lt;JWT&gt;
    Nginx->>Backend: Proxy /api/* → service
    Backend->>Keycloak: Fetch JWKS (cached)
    Keycloak-->>Backend: Public key
    Backend->>PG: Query / write
    PG-->>Backend: Result
